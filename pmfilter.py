...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import *
from info import LANGUAGES, LANGUAGE_ALIASES
...
def build_download_action_buttons(bot_username, ident, file_id, shortlink=False):
    """Return two clean action buttons for the clicked file row."""
    normal_link = f"https://t.me/{bot_username}?start={ident}_{file_id}"
    short_link = f"https://t.me/{bot_username}?start=short_{file_id}"

    if shortlink:
        return [
            InlineKeyboardButton("🍿 ꜱᴛʀᴇᴀᴍ", url=short_link),
            InlineKeyboardButton("📂 ᴅɪʀᴇᴄᴛ ꜰɪʟᴇ", url=normal_link),
        ]
    return [
        InlineKeyboardButton("🍿 ꜱᴛʀᴇᴀᴍ", url=normal_link),
        InlineKeyboardButton("📂 ᴅɪʀᴇᴄᴛ ꜰɪʟᴇ", url=normal_link),
    ]


def build_sendall_action_buttons(bot_username, key, shortlink=False):
    """Return two clean action buttons for the clicked Send All row."""
    normal_link = f"https://t.me/{bot_username}?start=allfiles_{key}"
    short_link = f"https://t.me/{bot_username}?start=sendfiles1_{key}"

    if shortlink:
        return [
            InlineKeyboardButton("🔗 ɢᴇᴛ ʟɪɴᴋ", url=short_link),
            InlineKeyboardButton("📦 ꜱᴇɴᴅ ᴀʟʟ", url=normal_link),
        ]
    return [
        InlineKeyboardButton("📦 ꜱᴇɴᴅ ᴀʟʟ", url=normal_link),
        InlineKeyboardButton("🚀 ᴏᴘᴇɴ ʙᴏᴛ", url=normal_link),
    ]


async def replace_clicked_button_row(query, new_row, fallback_text="⇊ ʏᴏᴜʀ ʟɪɴᴋ ɪs ʀᴇᴀᴅʏ ⇊"):
    """Replace the exact clicked inline-keyboard row with new buttons.

    This keeps the generated download/send buttons at the same place where
    the user clicked. If the old button cannot be found, the buttons are
    appended under the same bot message as a safe fallback.
    """
    ...
    if old_markup and old_markup.inline_keyboard:
        for row in old_markup.inline_keyboard:
            row_has_clicked_button = any(
                getattr(button, "callback_data", None) == query.data for button in row
            )
            if row_has_clicked_button and not replaced:
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: x.strip('[]{}()') and not x.strip('[]{}()').startswith('@') and not x.strip('[]{}()').startswith('www.'), [w.strip('[]{}()') for w in file.file_name.split()]))}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        return
    if lang != "homepage":
        await query.answer(f"✅ {language_display} ꜱᴇʟᴇᴄᴛᴇᴅ", show_alert=False)
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
    if settings["button"]:
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    selected = LANG_SELECTED.get(user_id, [])
    btn = []

    # Smart language UI: clean flag text, selected highlight, max 2 language select.
    for i in range(0, len(LANGUAGES), 2):
        row = []

        lang1 = LANGUAGES[i]
        if isinstance(lang1, (tuple, list)):
            text1, code1 = lang1[0], lang1[1]
        else:
            text1, code1 = lang1.title(), lang1.lower()
        text1 = f"{text1} ✅" if code1 in selected else text1
        row.append(InlineKeyboardButton(text=text1, callback_data=f"fl#{code1}#{key}"))

        if i + 1 < len(LANGUAGES):
            lang2 = LANGUAGES[i + 1]
            if isinstance(lang2, (tuple, list)):
                text2, code2 = lang2[0], lang2[1]
            else:
                text2, code2 = lang2.title(), lang2.lower()
            text2 = f"{text2} ✅" if code2 in selected else text2
            row.append(InlineKeyboardButton(text=text2, callback_data=f"fl#{code2}#{key}"))

        btn.append(row)

    selected_text = " + ".join(selected).upper() if selected else "ɴᴏɴᴇ"
    btn.insert(0, [InlineKeyboardButton("🌐 ꜱᴇʟᴇᴄᴛ ʟᴀɴɢᴜᴀɢᴇ | ᴍᴀx 2 🌐", callback_data="pages")])
    btn.insert(1, [InlineKeyboardButton(f"✅ ꜱᴇʟᴇᴄᴛᴇᴅ: {selected_text}", callback_data="pages")])
    btn.append([InlineKeyboardButton("✅ ᴀᴘᴘʟʏ ꜰɪʟᴛᴇʀ", callback_data=f"applylang#{key}")])
    btn.append([InlineKeyboardButton("🧹 ᴄʟᴇᴀʀ", callback_data=f"clearlang#{key}"), InlineKeyboardButton("🔙 ʙᴀᴄᴋ", callback_data=f"next_{user_id}_{key}_0")])
...
async def select_language_cb_handler(client: Client, query: CallbackQuery):
    ...
    if lang in selected:
        ...
        LANG_SELECTED[user_id] = selected
        await query.answer("☑️ Language removed")
    else:
        if len(selected) >= 2:
            return await query.answer("❌ ᴍᴀx 2 ʟᴀɴɢᴜᴀɢᴇꜱ ᴏɴʟʏ", show_alert=True)
        selected.append(lang)
        LANG_SELECTED[user_id] = selected
        await query.answer("✅ Language selected")

    query.data = f"languages#{key}"
    await languages_cb_handler(client, query)


@Client.on_callback_query(filters.regex(r"^clearlang#"))
async def clear_language_selection(client: Client, query: CallbackQuery):
    _, key = query.data.split("#")
    LANG_SELECTED[query.from_user.id] = []
    await query.answer("🧹 Selection cleared")
    query.data = f"languages#{key}"
    await languages_cb_handler(client, query)


@Client.on_callback_query(filters.regex(r"^applylang#"))
async def apply_language_filter_cb_handler(client: Client, query: CallbackQuery):
    _, key = query.data.split("#")
    selected = LANG_SELECTED.get(query.from_user.id, [])
    if not selected:
        return await query.answer("❌ ꜱᴇʟᴇᴄᴛ ᴀᴛ ʟᴇᴀꜱᴛ 1 ʟᴀɴɢᴜᴀɢᴇ", show_alert=True)
    ...
    for lang in selected:
        lang_query = LANGUAGE_ALIASES.get(lang.lower(), lang)
        search = f"{search} {lang_query}"
        display = lang.upper()
        for item in LANGUAGES:
            if isinstance(item, (tuple, list)) and item[1].lower() == lang.lower():
                display = item[0]
                break
    ...
    if not settings["button"]:
        ...
        try:
            ...
            pass
    await query.answer("✅ Applied: " + " + ".join(selected_names))
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            await replace_clicked_button_row(query, new_row)
            await query.answer("✅ Lɪɴᴋ ʙᴜᴛᴛᴏɴs ᴀʀᴇ ʀᴇᴀᴅʏ ʜᴇʀᴇ")
            return
        except MessageNotModified:
            await query.answer("✅ Lɪɴᴋ ᴀʟʀᴇᴀᴅʏ ʀᴇᴀᴅʏ")
            return
        ...
        try:
            ...
            await replace_clicked_button_row(
                query,
                new_row,
                fallback_text="⇊ ꜱᴇɴᴅ ᴀʟʟ ʟɪɴᴋ ɪs ʀᴇᴀᴅʏ ⇊",
            )
            await query.answer("✅ Sᴇɴᴅ Aʟʟ ʙᴜᴛᴛᴏɴs ᴀʀᴇ ʀᴇᴀᴅʏ ʜᴇʀᴇ")
            return
        except MessageNotModified:
            await query.answer("✅ Lɪɴᴋ ᴀʟʀᴇᴀᴅʏ ʀᴇᴀᴅʏ")
            return
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            for file in files:
                cap += f"<b>\n<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'> 📁 {get_size(file.file_size)} ▷ {' '.join(filter(lambda x: x.strip('[]{}()') and not x.strip('[]{}()').startswith('@') and not x.strip('[]{}()').startswith('www.'), [w.strip('[]{}()') for w in file.file_name.split()]))}\n</a></b>"
    else:
        if settings["button"]:
            ...
            for file in files:
                cap += f"<b><a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'> 📁 {get_size(file.file_size)} ▷ {' '.join(filter(lambda x: x.strip('[]{}()') and not x.strip('[]{}()').startswith('@') and not x.strip('[]{}()').startswith('www.'), [w.strip('[]{}()') for w in file.file_name.split()]))}\n\n</a></b>"
